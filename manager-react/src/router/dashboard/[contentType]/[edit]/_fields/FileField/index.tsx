...
import type { EncodedFileField } from '@rakun-kit/core/lib/fields/File'
...
type MediaRelationTranslatableValue =
  TranslatableValue<MediaRelationValue | null>
type MediaRelationArrayTranslatableValue =
  TranslatableValue<MediaRelationValue[] | null>
...
const isTranslatableMediaRelation = (
  value: unknown,
): value is MediaRelationTranslatableValue => {
  return (
    !!value &&
    typeof value === 'object' &&
    '_tag' in value &&
...
const FileField: React.FC<FileFieldProps> = ({ ref, ...props }) => {
  ...
  const [translations, setTranslations] =
    useState<MediaRelationTranslatableValue>(() =>
      props.isTranslatable &&
      !props.isMultiple &&
      isTranslatableMediaRelation(props.defaultData)
        ? props.defaultData
        : ({ _tag: 'Translatable' } as MediaRelationTranslatableValue),
    )
  const [translationsList, setTranslationsList] =
    useState<MediaRelationArrayTranslatableValue>(() =>
      props.isTranslatable &&
      props.isMultiple &&
  ...
  const [valueList, setValueList] = useState<MediaRelationValue[]>(() =>
    ...
    isTranslatableMediaRelationArray(props.defaultData)
      ...
      : !props.isTranslatable &&
          props.isMultiple &&
          isMediaRelationArray(props.defaultData)
        ? props.defaultData
        : [],
  )
  ...
  const defaultMediaListData = defaultMediaListRaw as
    | { items?: MediaRecord[] }
    | undefined
  const defaultMediaList = useMemo(
    () => ((defaultMediaListData?.items ?? []) as MediaRecord[]).filter(Boolean),
    [defaultMediaListData?.items],
  )
  ...
  const mediaListForPreview = useMemo(() => {
    if (selectedMediaList.length > 0) return selectedMediaList
    return defaultMediaList
  }, [defaultMediaList, selectedMediaList])
  ...
  const getValue = () => {
    ...
    if (props.isTranslatable) {
      ...
      const hasAnyValue = Object.entries(normalized).some(
        ([lang, relationValue]) =>
          lang !== '_tag' && isMediaRelation(relationValue),
      )
  ...
  const onSelectMedia = useCallback(async () => {
    const mediaOrList = await openMediaLibrary({
      ...
      optimizeOptions:
        props.uploadMethod === 'optimize' ? props.optimizeOptions : undefined,
    ...
    if (props.isMultiple) {
      const mediaList = (Array.isArray(mediaOrList)
        ? mediaOrList
        : [mediaOrList]
      ).filter((item) => isValidType(props.mediaType, item.mime))
            ...
            <Card className='p-3 text-sm'>
              <div className='space-y-1'>
                ...
                <div className='text-muted-foreground'>
                  {formatList(
                    mediaListForPreview.map((item) => item.name),
                    language.code,
                  )}
                </div>
              </div>
            ...
            <div className='flex items-center gap-3'>
              ...
              <div className='min-w-0'>
                <p className='truncate font-medium'>
                  {preview?.name || value._id}
                </p>
                <p className='text-muted-foreground'>
                  {preview?.mime || 'Unknown mime'}
                </p>
              </div>
            </div>
          </Card>

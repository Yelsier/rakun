...
const buildModuleTree = (root: HTMLElement): ModuleNavigationItem[] => {
  const elements = Array.from(
    root.querySelectorAll<HTMLElement>(moduleSelector),
  )
        ...
        <li key={item.id} className="relative pl-5">
          <span
            ...
            className={cn(
              ...
              index === items.length - 1 ? 'h-[1.125rem]' : 'h-full',
            )}
          />
          <span
            aria-hidden="true"
            className="absolute left-1.5 top-[1.125rem] h-px w-3.5 bg-border/70"
          />
          <Collapsible defaultOpen>
            <div
              className={cn(
                'group flex min-w-0 items-center rounded-lg transition-colors hover:bg-background/80',
            ...
            >
              <CollapsibleTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="ml-1 size-7 shrink-0 rounded-md text-muted-foreground data-[state=open]:[&_svg]:rotate-90"
              ...
              <Button
                variant="ghost"
                className="h-9 min-w-0 flex-1 justify-start gap-2 px-2 font-normal hover:bg-transparent"

    ...
    <section className="border-y border-zinc-200 bg-zinc-950 px-6 py-16 text-white">
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 flex flex-col gap-2">
          ...
          <h2 className="max-w-3xl text-3xl font-semibold tracking-normal">{title}</h2>
        </div>
            ...
            const content = (
              <div className="flex min-h-48 flex-col justify-between rounded-lg border border-white/10 bg-white/5 p-5 transition-colors hover:bg-white/10">
                <div className="space-y-3">
                  ...
                  <p className="text-sm leading-6 text-zinc-300">{text(item.summary)}</p>
                </div>
                {href ? (
                  <span className="mt-6 text-sm font-semibold text-emerald-300">{href}</span>
                ) : null}
              </div>
            )

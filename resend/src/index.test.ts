import { describe, expect, test } from 'bun:test'
import type { Resend } from 'resend'

import type { MailMessage } from '@rakun-kit/core'

import { ResendMailAdapter, createResendMailServiceConfig } from './index'

const message: MailMessage = {
  from: { address: 'sender@example.com', name: 'Rakun' },
  to: [{ address: 'one@example.com' }],
  cc: [{ address: 'copy@example.com', name: 'Copy' }],
  bcc: [{ address: 'blind@example.com' }],
  replyTo: [{ address: 'reply@example.com' }],
  subject: 'Welcome',
  html: '<p>Hello</p>',
  text: 'Hello',
  headers: { 'X-Rakun': 'mail' },
  attachments: [
    {
      filename: 'invoice.pdf',
      content: new Uint8Array([1, 2, 3]),
      contentType: 'application/pdf',
      contentId: 'invoice',
    },
  ],
}

describe('ResendMailAdapter', () => {
  test('maps the normalized message to Resend', async () => {
    const calls: Record<string, unknown>[] = []
    const client = {
      emails: {
        send: async (input: Record<string, unknown>) => {
          calls.push(input)
          return { data: { id: 'resend-1' }, error: null }
        },
      },
    } as unknown as Resend
    const adapter = new ResendMailAdapter('re_test', client)

    await expect(adapter.send(message)).resolves.toEqual({ id: 'resend-1' })
    expect(calls[0]).toMatchObject({
      from: 'Rakun <sender@example.com>',
      to: ['one@example.com'],
      cc: ['Copy <copy@example.com>'],
      bcc: ['blind@example.com'],
      replyTo: ['reply@example.com'],
      subject: 'Welcome',
      html: '<p>Hello</p>',
      text: 'Hello',
      headers: { 'X-Rakun': 'mail' },
    })
    expect(calls[0]?.attachments).toEqual([
      {
        filename: 'invoice.pdf',
        content: Buffer.from([1, 2, 3]),
        contentType: 'application/pdf',
        contentId: 'invoice',
      },
    ])
  })

  test('surfaces provider errors without exposing the API key', async () => {
    const client = {
      emails: {
        send: async () => ({
          data: null,
          error: { message: 'Invalid sender' },
        }),
      },
    } as unknown as Resend
    const adapter = new ResendMailAdapter('re_secret', client)

    await expect(adapter.send(message)).rejects.toThrow(
      'Resend failed to send mail: Invalid sender'
    )
    await expect(adapter.send(message)).rejects.not.toThrow('re_secret')
  })

  test('creates a core service config', () => {
    const config = createResendMailServiceConfig({
      apiKey: 're_test',
      defaultFrom: 'sender@example.com',
      defaultReplyTo: 'reply@example.com',
    })

    expect(config.adapter).toBeInstanceOf(ResendMailAdapter)
    expect(config.defaultFrom).toBe('sender@example.com')
    expect(config.defaultReplyTo).toBe('reply@example.com')
  })
})

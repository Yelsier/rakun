...
const toYValue = (value: unknown): unknown => {
  if (value instanceof Date) {
    return new Y.Text(value.toISOString())
  }
  ...
  if (Array.isArray(value)) {
    ...
    if (value.length > 0) result.insert(0, value.map(toYValue))
    return result
  }
...
export const initializeContentDocument = (
  doc: Y.Doc,
  ...
  doc.transact(() => {
    for (const [key, value] of Object.entries(snapshot)) {
      if (value !== undefined) root.set(key, toYValue(value))
    }
  })
}

export const setContentField = (doc: Y.Doc, field: string, value: unknown) => {
  const root = doc.getMap<unknown>(CONTENT_ROOT_NAME)
  doc.transact(() => {
    if (value === undefined) {
      root.delete(field)

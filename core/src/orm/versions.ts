...
type VersionedContentType = Pick<ContentType, 'name' | 'schemaVersion' | 'versioning'>
...
const diffValues = (before: unknown, after: unknown, path = ''): VersionDiffEntry[] => {
...
const toContentVersionRecord = (value: Document | null): ContentVersionRecord | null => {
...
const loadActorInfo = async (db: Db, actorIds: readonly (string | undefined)[]) => {
  ...
  return new Map(
    users.map((user) => {
      ...
      const avatarId = user.avatarId ? String(user.avatarId) : undefined
      const label =
      ...
      return [
        id,
        {
          ...
          avatar: avatarId
            ? {
                _id: avatarId,
                key: typeof user.avatarKey === 'string' ? user.avatarKey : undefined,
                access:
                  user.avatarAccess === 'public' || user.avatarAccess === 'private'
                    ? user.avatarAccess
                    : undefined,
                url: typeof user.avatarUrl === 'string' ? user.avatarUrl : undefined,
                previewUrl:
                  typeof user.avatarPreviewUrl === 'string' ? user.avatarPreviewUrl : undefined,
              }
            : undefined,
        },
      ] as const
...
const enrichVersionRecords = async (db: Db, records: ContentVersionRecord[]) => {
  ...
  return records.map((record) => {
    const actor = record.actorId ? actorInfo.get(record.actorId) : undefined

    return actor ? { ...record, actorLabel: actor.label, actorAvatar: actor.avatar } : record
  })
}

const enrichVersionRecord = async (db: Db, record: ContentVersionRecord | null) =>
...
const getMaxVersions = (contentType: VersionedContentType) =>
  typeof contentType.versioning === 'object' ? contentType.versioning.maxVersions : undefined

const pruneVersions = async (db: Db, contentType: VersionedContentType, documentId: string) => {
...
export const recordContentVersion = async (
  ...
  const revision =
    options.revision ?? Number(after?._revision ?? Number(before?._revision ?? 0) + 1)
  ...
  const version = await db.collection(VERSIONS).findOne({ _id: insert.insertedId })
...
export const createMongoVersionAdapter = (db: Db): VersionAdapter => {
  const list = async (input: VersionListInput): Promise<ContentVersionRecord[]> => {
  ...
  const get = async (versionId: string): Promise<ContentVersionRecord | null> => {
    ...
    const version = await enrichVersionRecord(
      db,
      toContentVersionRecord(await db.collection(VERSIONS).findOne({ _id: parseId(versionId) }))
    )
  ...
  const restore = async (input: VersionRestoreInput): Promise<VersionRestoreOutput> => {
    ...
    return {
      version,
      restored: transformObjectIdsToStrings(restoredDocument) as Record<string, unknown>,
    }
  }

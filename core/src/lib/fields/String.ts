...
type StringValue<Ui extends StringUI> = Ui extends 'RichText' ? Record<string, unknown> : string
...
type StringFieldCore<Ui extends StringUI, State extends FieldState> = StringFieldBase<Ui, State> & {
  type: <TThis extends StringFieldBase<Ui, FieldState>, NextUi extends StringUI>(
    this: TThis,
    ui: NextUi
...
type StringFieldBase<Ui extends StringUI, State extends FieldState> = FieldLike<
  StringValue<Ui>,
  StringValue<Ui>,
  StringValue<Ui>,
  StringMeta<Ui>,
...
function makeStringField<Ui extends StringUI, State extends FieldState>(
  ...
  const meta = {
    ...
    seo: options.seo,
    capabilities: {
      valueKind: options.ui === 'RichText' ? 'richText' : 'string',
    },
  ...
  const field: StringFieldCore<Ui, State> = {
    ...
    type: function <TThis extends StringFieldBase<Ui, FieldState>, NextUi extends StringUI>(
      this: TThis,
      ui: NextUi
    ) {
      ...
      return makeStringField({ ...options, ui }, this.state as FieldStateOf<TThis>)
    },
    min: function <TThis extends StringFieldBase<Ui, FieldState>>(this: TThis, length: number) {
      return makeStringField({ ...options, minLength: length }, this.state as FieldStateOf<TThis>)
    },
    max: function <TThis extends StringFieldBase<Ui, FieldState>>(this: TThis, length: number) {
      return makeStringField({ ...options, maxLength: length }, this.state as FieldStateOf<TThis>)
    },
    seo: function <TThis extends StringFieldBase<Ui, FieldState>>(
      ...
      return makeStringField({ ...options, seo }, this.state as FieldStateOf<TThis>)
    },
  }
  ...
  return withFieldModifiers({
    ...
    rebuild: <NextState extends FieldState>(nextState: NextState) =>
      makeStringField(options, nextState) as WithFieldState<StringFieldCore<Ui, State>, NextState>,
  })
}
...
function buildStringSchema<Ui extends StringUI>(
  ...
  if (options.ui === 'RichText') {
    return z.record(z.string(), z.unknown()) as unknown as z.ZodType<StringValue<Ui>>
  }

  let schema = options.ui === 'Email' ? z.email() : options.ui === 'Url' ? z.url() : z.string()

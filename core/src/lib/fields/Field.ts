...
export const FieldValueKind = z.enum([
  'string',
  'richText',
  'number',
  'boolean',
...
export type FieldValueKind = z.infer<typeof FieldValueKind>

export const FieldCollectionKind = z.enum(['homogeneous', 'heterogeneous'])

export type FieldCollectionKind = z.infer<typeof FieldCollectionKind>

export type FieldDynamicCapabilities = {
  /** Named leaf values exposed by object-like fields to dynamic data. */
  properties?: Readonly<Record<string, FieldValueKind>>
  /** Allows those properties to be mapped independently on a target field. */
  mapProperties?: boolean
  /** Describes fields whose nested content type can be traversed. */
  relation?: boolean
  /** Enables per-item list mapping for a collection field. */
  collection?: FieldCollectionKind
}

export type FieldCapabilities = {
  /** Serializable value shape used for dynamic-data compatibility. */
  valueKind: FieldValueKind
  dynamic?: FieldDynamicCapabilities
}

export type FieldPopulateContext = {
  db: DBService
  populate: (value: unknown, field?: AnyFieldLike) => Promise<unknown>
  populateLink: (value: unknown) => Promise<unknown>
}

export type FieldRuntime = {
  /** Server-only population hook. It is never encoded for the manager. */
  populate?: (value: unknown, context: FieldPopulateContext) => Promise<unknown> | unknown
}
...
export type SetTranslatable<State extends FieldState> = Omit<State, 'translatable'> & {
  translatable: true
}

export type SetVisibility<State extends FieldState, V extends Visibility> = Omit<
  State,
  'visibility'
> & {
  visibility: V
...
export type SetCondition<State extends FieldState, Condition extends FieldCondition> = Omit<
  State,
  'condition'
> & {
  condition: Condition
...
export type SetDescription<State extends FieldState> = Omit<State, 'description'> & {
  description: string
}

export type SetDynamic<State extends FieldState, Dynamic extends boolean> = Omit<
  State,
  'dynamic'
> & {
  dynamic: Dynamic
...
type MaybeTranslatable<Value, State extends FieldState> = State['translatable'] extends true
  ? TranslatableValue<Value>
  : Value

type MaybeInput<Value, State extends FieldState> = State['condition'] extends FieldCondition
  ? Value | null | undefined
  : State['required'] extends true
    ? Value
    : Value | null | undefined

type MaybeOutput<Value, State extends FieldState> = State['required'] extends true
  ? Value
  : Value | undefined

export type FieldInput<InputValue, State extends FieldState> = State['visibility'] extends 'api'
  ? undefined
  : MaybeInput<MaybeTranslatable<InputValue, State>, State>

export type FieldDb<DbValue, State extends FieldState> = State['visibility'] extends 'api'
  ? undefined
...
> = State['visibility'] extends 'manager' ? undefined : MaybeOutput<OutputValue, State>
...
export type FieldValueOf<F> = F extends { readonly __fieldTypes?: { input: infer Value } }
  ? Value
  : never

export type FieldDbValueOf<F> = F extends { readonly __fieldTypes?: { db: infer Value } }
  ? Value
  : never

export type FieldOutputValueOf<F> = F extends { readonly __fieldTypes?: { output: infer Value } }
  ? Value
  : never

export type FieldMetaOf<F> = F extends { readonly __fieldTypes?: { meta: infer Meta } }
  ? Meta
  : never

export type FieldStateOf<F> = F extends {
  readonly __fieldTypes?: { state: infer State extends FieldState }
}
  ? State
  : never
...
export type AnyFieldLike = FieldLike<unknown, unknown, unknown, FieldMetaBase, FieldState>

export type PopulatableFieldLike<PopulatedValue, State extends FieldState = DefaultFieldState> = {
...
type RebindPopulatedField<F extends AnyFieldLike, State extends FieldState> =
...
export type FieldWithModifiers<F extends AnyFieldLike> = F & {
  ...
  managerOnly: () => WithFieldState<F, SetVisibility<FieldStateOf<F>, 'manager'>>
  noDynamic: () => WithFieldState<F, SetDynamic<FieldStateOf<F>, false>>
  description: (description: string) => WithFieldState<F, SetDescription<FieldStateOf<F>>>
...
export type WithFieldState<F extends AnyFieldLike, State extends FieldState> = FieldWithModifiers<
  Omit<F, keyof AnyFieldLike | FieldModifierKeys> &
    FieldLike<FieldValueOf<F>, FieldDbValueOf<F>, FieldOutputValueOf<F>, FieldMetaOf<F>, State> &
...
export type InferDb<F> =
  F extends FieldLike<any, infer Value, any, unknown, infer State> ? FieldDb<Value, State> : never
...
export type InferPopulated<F> = F extends { getPopulatedSchema: () => z.ZodType<infer Value> }
  ? Value
  : InferDb<F>
  ...
  return {
    ...
    getInputSchema: () =>
      applyManagerVisibility(
        applyInputPresence(applyTranslatable(params.schemas.input(), params.state), params.state),
        params.state
      ) as z.ZodType<FieldInput<InputValue, State>>,
    getSchema: () =>
      applyManagerVisibility(
        applyOutputPresence(applyTranslatable(params.schemas.db(), params.state), params.state),
        params.state
...
>(params: {
  ...
  runtime?: FieldRuntime
}): FieldWithModifiers<FieldLike<InputValue, DbValue, OutputValue, Meta, DefaultFieldState>> {
  const makeField = <State extends FieldState>(
    state: State
  ): FieldWithModifiers<FieldLike<InputValue, DbValue, OutputValue, Meta, State>> => {
...
export function withFieldModifiers<F extends AnyFieldLike>(params: {
  field: F
  rebuild: <NextState extends FieldState>(state: NextState) => WithFieldState<F, NextState>
...
function applyTranslatable<Value>(schema: z.ZodType<Value>, state: FieldState) {

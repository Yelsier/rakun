...
type SelectOptions<Options extends readonly string[], Multiple extends boolean> = {
  options: Options
  multiple: Multiple
  minItems?: number
  maxItems?: number
...
> = SelectFieldBase<Options, Multiple, State> & {
  multiple: <TThis extends SelectFieldBase<Options, Multiple, FieldState>>(
    this: TThis
...
function makeSelectField<
  ...
  Multiple extends boolean,
  State extends FieldState,
>(options: SelectOptions<Options, Multiple>, state: State): SelectField<Options, Multiple, State> {
  const field: SelectFieldCore<Options, Multiple, State> = {
    ...createField({
      meta: {
        type: 'Select',
        ui: (options.multiple ? 'MultiSelect' : 'Select') as SelectMeta<Options, Multiple>['ui'],
    ...
    multiple: function <TThis extends SelectFieldBase<Options, Multiple, FieldState>>(this: TThis) {
      return makeSelectField({ ...options, multiple: true }, this.state as FieldStateOf<TThis>)
    },
    min: function <TThis extends SelectFieldBase<Options, Multiple, FieldState>>(
      this: Multiple extends true ? TThis : never,
      count: number
    ) {
      return makeSelectField({ ...options, minItems: count }, this.state as FieldStateOf<TThis>)
    },
    max: function <TThis extends SelectFieldBase<Options, Multiple, FieldState>>(
      this: Multiple extends true ? TThis : never,
      count: number
    ) {
      return makeSelectField({ ...options, maxItems: count }, this.state as FieldStateOf<TThis>)
    },
  }
...
function buildSelectSchema<Options extends readonly string[], Multiple extends boolean>(
  options: SelectOptions<Options, Multiple>
) {
  const literal = z.literal(options.options.length ? options.options : ['']) as z.ZodType<
    Options[number]
  >
  ...
  return (options.multiple ? list : literal) as z.ZodType<SelectValue<Options, Multiple>>
}

...
const escapeLabel = (value: string) => normalizeInline(value).replace(/([\\\[\]])/g, '\\$1')
...
const escapeHref = (value: string) => value.replace(/\(/g, '%28').replace(/\)/g, '%29')

const renderEntry = (entry: LlmsRenderedEntry) => {
  const description = entry.description ? `: ${normalizeInline(entry.description)}` : ''
...
export const renderLlmsTxt = (document: LlmsRenderedDocument): string | undefined => {
...
): unknown => (isTranslatableObject(value) ? getTranslation(value, language, languages) : value)
...
const getDefaultSeo = (settings: UnknownRecord | undefined) => unwrapRelation(settings?.defaultSeo)
...
const buildRenderedEntry = ({
  ...
  const localizedLink = getLocalizedValue(entry.llmsLink, language, languages)
  const link = isRecord(localizedLink) ? localizedLink : undefined
  const rawHref = link ? getLocalizedValue(link.href, language, languages) : localizedLink
  const href = typeof rawHref === 'string' ? normalizeHref(rawHref, siteUrl) : undefined
  if (!href) return undefined

  const title = (link ? getLocalizedString(link.title, language, languages) : '') || href
  const description = getLocalizedString(entry.llmsLinkDescription, language, languages)
...
export const getLlmsTxt = async (input: LlmsInput = {}): Promise<LlmsOutput> => {
  ...
  const siteUrl = typeof seoSettings?.siteUrl === 'string' ? seoSettings.siteUrl : undefined
  const title =
  ...
  const details = getLocalizedString(populated.llmsDetails, language, languages)
  const sections = getBlockValues(populated.llmsSections, 'LlmsSection').map((section) => ({
    title: getLocalizedString(section.llmsSectionTitle, language, languages),
    optional: section.llmsOptional === true,
    entries: getBlockValues(section.llmsEntries, 'LlmsEntry').flatMap((entry) => {

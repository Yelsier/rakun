...
import type { AnyFieldLike } from '../../../lib/fields/Field'
...
export const populateFields = async <T extends ContentType>(
  data: DBOutput<T>
): Promise<DataFront<T>> => {
  ...
  const populateLinkValue = async (value: unknown): Promise<unknown> => {
    ...
    if (value && typeof value === 'object' && 'routeId' in value && 'contentTypeId' in value) {
      ...
      const populated = Object.fromEntries(
        items
          .map((item) => {
            const lang = languages.find((l) => String(l._id) === String(item.languageId))
  ...
  const populateValue = async (value: unknown, field?: AnyFieldLike): Promise<unknown> => {
    ...
    if (field?.runtime.populate) {
      return field.runtime.populate(value, {
        db,
        populate: populateValue,
        populateLink: populateLinkValue,
      })
    }
...
/** @deprecated Use populateFields. Kept as a public compatibility alias. */
export const populateLinks = populateFields
